...
keep-selection-each-line %{ evaluate-commands %sh{
...
drop-selection-each-line %{ evaluate-commands %sh{
	if [ $kak_count -eq 0 ] ; then
...
#sort command use \n as separator therefore use tr ' ' '\n'
define-command -hidden _keep-last-selection-each-line %{ evaluate-commands %sh{
...
define-command -hidden _keep-nth-selection-each-line %{ evaluate-commands %sh{
	old_selections=`echo $kak_selections_desc | tr ' ' '\n' | sort -t '.' -k 1n,1 -k 2n,2`
	selections=
	prev_line=`echo $old_selections | cut -f 1 -d '.'`
	i=1
...
define-command -hidden _drop-last-selection-each-line %{ evaluate-commands %sh{
	old_selections=`echo $kak_selections_desc | tr ' ' '\n' | sort -t '.' -k 1n,1 -k 2n,2`
	selections=
	prev_line=`echo $old_selections | cut -f 1 -d '.'`
	last_sel=
...
define-command -hidden _drop-nth-selection-each-line %{ evaluate-commands %sh{
	old_selections=`echo $kak_selections_desc | tr ' ' '\n' | sort -t '.' -k 1n,1 -k 2n,2`
	selections=
	prev_line=`echo $old_selections | cut -f 1 -d '.'`
	i=1
